...
| 5 | Re-randomizable AH encryption (B) | Game layer: B-rerand's presented ciphertext is modeled as a fresh sample independent of the receipt chain (the re-randomization guarantee, taken as the ideal functionality); B-static's is the literal chain value — the difference is the entire content of the T4 calibration pair. |
