...
| 5 | Re-randomizable AH encryption (B) | `Crypto.MaskedEncryption` gives an executable additive opening-carrying reference scheme and proves exact rerandomized-presentation independence; the B game uses that resulting fresh-uniform law. A deployed public-key reduction remains outside the current ideal/reference layer. |
