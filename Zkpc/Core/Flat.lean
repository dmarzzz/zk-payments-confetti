...
/-!
# Flat-ticket instantiation of the abstract object (task D5; Spec.md §3)
...
1. *Check 1 (π verifies).* Proof objects do not exist in the model; a
   ticket is its witness, so verification is the guard "the components
   satisfy `R_spend`" (root/liveness, solvency) checked semantically.
...
6. *`Dispute`'s claims window.* `dispute` records the slash and its time
   ...
   with T6/T7).
7. *`merge` (MC17)* is degenerate at `N = 1` (a gateway never merges its
   own tuples) but implemented per §2: conflicting incoming tuples
   produce evidence, fresh ones join the spent set.
8. *`noncomputable`.* The evidence branches of `redeem`/`merge` extract
...
/-- **Flat-ticket instantiation A (Spec.md §3) of the abstract scheme
(Spec.md §2).** Carriers from the symbolic machine: secrets in the signal
field `F`, tickets/spent tuples as extracted witnesses `(k, i, m)`,
evidence as a conflicting signal pair on one nullifier, ledger state the
machine state of `Zkpc.Core.St`. `Pl` is the request-payload sort and
`mclose` the distinguished close message (MC1). See the file GATE-NOTE
for the checks the deterministic layer carries as guards. -/
noncomputable def flatScheme (F : Type) [Field F] [DecidableEq F]
    (Pl : Type) [DecidableEq Pl] (mclose : Msg Pl) : Zkpc.Spec.Scheme F where
  Gateway := Unit
  Payload := Pl
  Ticket := F × ℕ × Msg Pl
  Receipt := Unit
  ...
  -- Fleet reconciliation (MC17), degenerate at `N = 1` (GATE-NOTE 7):
  ...
  -- Payer close ("close-as-final-spend", MC1): the close signal at the
  -- payer's next unused index on `m_close`, opening the window at the
  -- current time (`Step.payerClose`; settlement at expiry is the
  -- machine's `settleClose`, automatic per §2).
  payerClose := fun _pp stP ledger =>
    match stP with
    | (k, j, _) =>
      if ledger.live k then
        some { ledger with
          sigs := insert (k, j, mclose) ledger.sigs
          closedAt := Function.update ledger.closedAt k (some (j, ledger.clock)) }
      else none
  -- Payee close ("sweep", MC16): pay `C` per fresh nullifier of an
  -- accepted tuple, deduped against `RedeemedNF`, post-slash only
  -- within the priority window (`Step.sweepOne`'s three guards,
  -- `sweepOpen` from Zkpc.Core.T2). Non-sweepable tuples are skipped.
  sweep := fun pp _gw tuples ledger =>
    tuples.foldl
      (fun st u =>
        match u with
        | (k, i, _m) =>
          if u ∈ st.acc ∧ (k, i) ∉ st.swept ∧ sweepOpen pp.tau st k then
