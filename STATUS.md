...
| New protocol (non-frameability kernel) | Stage 1 of obligation 3: the secret-averaged hidden-target bound `q/\|C\| + 1/\|N\|` for framing an honest close, with must-lose calibrations (leaky next-nullifier, grindable secret space). Stage 2 (adaptive shared-oracle composition, the FrameDeferred port) open. | `lean/Zkpc/Chain/V2/Frame.lean` |
...
| New protocol (close-window liveness, obligation 5) | Guaranteed progress for both parties: Bob cannot disable Alice's canonical safe close from any reachable live state; Bob is never locked out (timeout enabled past `Tabs`, or past `Treq` after a request); no reachable live state is stuck (`not_stuck`) | `lean/Zkpc/Chain/V2/Liveness.lean` |
| New protocol (non-frameability, stage 2 core) | Adaptive `q`-probe secret-recovery bound `q/\|C\|` (hidden-target game) summed with the fresh-target `1/\|N\|` | `lean/Zkpc/Chain/V2/FrameAdaptive.lean` |
