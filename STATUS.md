...
| About | What | Where |
|---|---|---|
| New protocol (Spec-v2 draft, safety core) | Clocked machine with explicit challenge window and evidence-based challenge guard (the seed's three fiats discharged); conservation; no-overspend; cooperative-settlement exactness and the safe-close payout floor; the safe-close characterization (genesis iff nothing sent, signed tip iff no ghost, the ghost itself, fresh tip extension); vigilant-Bob challenge-enabledness iff the close is unsafe; safe-exit liveness for Alice including the withheld-countersignature wedge, at a price of one δ (the G2 repair, conditioned on Spec-v2 §3 frontier injectivity). NOTE: the settlement-safety theorems are conditional on the two probabilistic kernels below (collision-freedom, non-frameability); the fused end-to-end bound is the unfused residual | `lean/Zkpc/Chain/V2/` |
...
| New protocol (non-frameability kernel) | Stage 1 of obligation 3: the secret-averaged hidden-target bound `q_C/\|C\| + q_N/\|N\|` (probes + a per-held-message guess list) for framing an honest close, with must-lose calibrations. Stage 2 adaptive *secret-probe* term done; the fused adaptive game (FrameDeferred port) open. | `lean/Zkpc/Chain/V2/Frame.lean` |
| New protocol (anonymity calibration, obligation 4 anti-vacuity) | The must-catch calibration: a linkable-nullifier scheme (mask dropped from the next-nullifier commitment) has distributionally distinct worlds (`linkable_leak_detected`), the exact negation of the real scheme's coupling — the game is not vacuous. Must-pass counterpart is `chain_two_payment_anonymity`. | `lean/Zkpc/Chain/V2/CalibrationChain.lean` |
| New protocol (close-window enabledness, obligation 5 partial) | Enabledness + deadlock-freedom (NOT full guaranteed-liveness — the fairness/temporal wrapper is open): Bob cannot disable Alice's canonical safe close from any reachable live state; Bob is never locked out (timeout enabled past `Tabs`, or past `Treq` after a request); no reachable live state is stuck (`not_stuck`, progress-possible not progress-forced) | `lean/Zkpc/Chain/V2/Liveness.lean` |
| New protocol (non-frameability, stage 2 secret-probe term) | Adaptive `q`-probe secret-recovery bound `q/\|C\|` (hidden-target game); stated with the fresh-target term as two *separate* bounds, not a fused game (the fusion is the FrameDeferred residual) | `lean/Zkpc/Chain/V2/FrameAdaptive.lean` |
| New protocol (close-view anonymity, stage 1 of obligation 4) | Signed-close unlinkability, advantage exactly 0 (equal-split session form, ideal model): the close view is simulatable independently of which channel it settles. The proof attempt found and repaired F-R2-1 (the round-1 close published `C_x`, an attribution leak). Later stages: adaptive adversary, session vectors, ghost-close view, calibration battery. | `lean/Zkpc/Chain/V2/CloseView.lean` |
