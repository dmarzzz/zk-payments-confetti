...
bound (under a stated random-oracle good-event hypothesis, with the query
tail deferred), and the refund variant's safety and conservation theorems
are machine-checked with zero \texttt{sorry} and no axiom beyond Lean's
three standard ones. The definition went through eleven rounds of
...
\item \textbf{T7 --- exculpability (framing) bound} ---
  ...
  $1/|F|$ --- the blind-guess floor. Honestly in-place: the $q/|F|$
  query-term accounting that would discharge \decl{hobliv} for an unbounded
  interactive adversary is the deferred PPT tail (GATE-NOTE in
  \texttt{T7.lean}); the kernel guarantees everything up to
  \decl{hobliv}, and we do not claim the unconditional bound. Two
  \emph{must-win} calibration adversaries confirm the game has teeth:
  \decl{frameWinProb\_YK\_eq\_one} (degenerate $y = k$) and
  \decl{frameWinProb\_aReuse\_eq\_one} ($a$ reused across indices) each
  frame with probability exactly $1$.
...
\paragraph{Machine-checked refund-variant safety.} The refund variant
...
The model here is single-channel and models \emph{one} close-dispute round;
the full failed-upgrade cascade (one count restored per round, converging
at the true count) is a documented deferral (GATE-NOTE), not claimed.
...
occur: T7's PPT query tail behind \decl{hobliv}, and the refund upgrade
cascade beyond the single round modeled. The B-instantiation UNLINK result
is the ideal-model calibration pair; its model-to-real bridges
(\decl{zkBridgeObligation}, the genesis obligations) are stated
obligations, discharged per the named assumptions rather than against a
concrete SNARK/encryption scheme --- circuits are out of the model boundary
(\S\ref{sec:boundary}).
...
T7 (framing) & \texttt{Zkpc/Games/T7.lean}, \texttt{Zkpc/Games/Frame.lean} & \decl{T7\_frame\_bound} ($\le 1/|F|$ under \decl{hobliv}), \decl{frameWinProb\_YK\_eq\_one}, \decl{frameWinProb\_aReuse\_eq\_one}, \decl{frameGame}, \decl{frameWinProb}, \decl{Slashes} \\
