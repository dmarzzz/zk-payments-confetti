...
- **L** = Lean declaration (file + name; machine-checked at the pinned
  toolchain, verified by `lake build` + CI guardrails).
...
| 5.22 | One-trace composition PROVED: `channel_endToEnd_composition` (settlement + exact floor + T1 + T2 + exculpability simultaneously on one reachable trace) and `wire_endToEnd_composition` (T4 = 0 together with the zero-loss ZK bridge for the FS wire family) (`Zkpc/Core/Composition.lean`) | L; K2 |
