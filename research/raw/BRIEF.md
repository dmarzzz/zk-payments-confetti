...
The ZK API Usage Credits construction (ethresear.ch/t/24104) lets a client deposit once and spend anonymously against one provider, with double-spend detection via nullifiers. The construction is, structurally, a zk payment channel bound to one recipient (the channel mapping is stated in the thread itself, and ../processed/field-report.md verifies it against BOLT), and zk payment channels have essentially no dedicated literature. This brief scopes the two artifacts that would create that literature: a systematization paper and a machine-checked formalization.

Companion input: `../processed/field-report.md` (field report from a verified research sweep across BOLT/zkChannels, hub privacy, Chaumian ecash, state-channel capital efficiency, and the formal-verification landscape). Read it first; do not re-derive the map.
...
- T6 **Priced divergence (fleet).** Across N gateways sharing the spent set with reconciliation lag L and per-epoch rate limit r, the value a member can extract by double-spending before detection is bounded by a function of (L, r) that stays below the deposit D. This is the theorem that justifies "the async window is priced by the deposit"; it is open problem 1 in ../processed/field-report.md and it is the fleet's actual security argument.
...
M0's `Spec.md` should formalize *this* protocol, not a reinvented one: its prose sources are the wire-protocol and application sections of the egress post (reputation-gated-egress.vercel.app) and the application section of ../processed/field-report.md. Toolchain is pinned in `lean-toolchain` and `lakefile`; CI runs `lake build` on a fixed mathlib + VCV-io revision.
