...
`gates.md`. This document is the trust surface of the
...
../processed/field-report.md's application section — not a reinvention. Where those sources
...
per-index private windows); it is kept in `gates.md` as
...
in `gates.md`). **The M0 reviewer is asked to check every
item.**
...
- **MC1 — Close mechanics. [repair; superseded in part by MC20]**
  ...
  the source construction (../processed/field-report.md open problem 8; the thread comment
...
- **MC4 — Slash payout with gateway-priority window. [repair]** The source
  ...
  as written admits the self-slash race (../processed/field-report.md open problem 2,
...
- **MC9 — Domain separation assumed.** The dual-nullifier circuit (index
  ...
  nothing) is open problem 4 in ../processed/field-report.md; we *assume* domain-separated
...
| Open problems this spec commits on: gossip-lag bound, self-slash race, deposit-edge leak | Egress post, Addendum (final paragraph); ../processed/field-report.md open problems 1, 2, 3, 8 |
| RLN algebra $a = H(k,i)$, $x = H(M)$, $y = k + a x$, $nf = H(a)$; index in the role of RLN's epoch; "different $x$, same nullifier" detection; $k$ recovery; slash claimable by anyone | ../processed/field-report.md, "What zk payment channel actually means" item 3 and deep dive 1 (verified against the ZK API Usage Credits thread) |
| Solvency invariants $(i+1) C \le D$ (flat) and $(i+1) C_{max} \le D + R$ (refunds) | ../processed/field-report.md deep dives 1 and Application section; BRIEF.md instantiation list |
| Refund variant: server-signed refund tickets / encrypted running total; static $E(R)$ linkable (omarespejel), re-randomization with proof of equivalence as the patch | ../processed/field-report.md deep dive 1; BRIEF.md T4 calibration requirement |
| Abort/evict oracle requirement and its two attack modes (evict to shrink the set; abort mid-sequence to link) | BOLT §1.4 as quoted in ../processed/field-report.md deep dive 2; BRIEF.md T4 |
| Anonymity-set accounting (set = capable/completed participants, shrinking under aborts) informing the $\bot$-branch of T4 | ../processed/field-report.md deep dives 2, 3 |
...
| Exculpability shape: forging a second point on the line requires the secret; $N-1$ collusion | ../processed/field-report.md open problem 5; BRIEF.md T7 |
| Priced-divergence shape: extractable value $\le f(\text{lag}, \text{rate}) < D$ | ../processed/field-report.md open problem 1; BRIEF.md T6; egress post Addendum |
| Self-slash race and mitigation directions (frequent claims / unclaimed-balance ceiling) informing MC4 | ../processed/field-report.md deep dive 1 (e), open problem 2, and Application "design imports" |
| Dual stake $D + S$ (excluded, MC8) | ../processed/field-report.md deep dive 1 |
...
| MC15/MC16/MC17 and the MC7 expansion: protocol/game repairs forced by rev-1 agent-review counterexamples | `gates.md` (B1 gate record) |
| MC18 close-time netting for B and MC19 checkpointed window claims: repairs forced by rev-2 counterexamples (NEW-1, NEW-2), refined by rev-3 (R3-1..R3-3) | `gates.md` (B1 gate record, rounds 2–3) |
| MC20 verifiable spend count at close (A enumeration + B certified count with $nf_j$ reveal, receipt-bearing disputes, upgrade sub-window, two-sided sweep bar): repairs forced by rev-5/6/7 counterexamples | `gates.md` (B1 gate record, rounds 5–7) |
...
in ../processed/field-report.md; this spec cites it through ../processed/field-report.md deliberately, since
../processed/field-report.md is the verified field report the executor contract designates as
the map.
